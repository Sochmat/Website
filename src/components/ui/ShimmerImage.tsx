...
export default function ShimmerImage({
  ...
  // A cached image can finish decoding before React attaches its handlers, so
  // onLoad never fires and the shimmer would sit on top of a perfectly good
  // picture forever. Checking `complete` on mount closes that race; the
  // naturalWidth test is what separates "done" from "done, but broken".
  useEffect(() => {
 ...
 * already sized in Blob storage and every one of them is `unoptimized`, so the
 * wrapper would buy nothing. This keeps those call sites a one-word change
 * instead of a layout rewrite.
 *
  ...
  return (
    <>
      ...
      <img
        ...
        // placeholder dish is better than an alt-text stub on a menu card.
        src={state === "error" ? fallbackSrc : props.src}
        className={`${className} transition-opacity duration-300 ${
          state === "loaded" || state === "error" ? "opacity-100" : "opacity-0"
        }`}
        onLoad={() => setState("loaded")}
        onError={() => setState((s) => (s === "error" ? s : "error"))}
      />
    </>
  );
}
