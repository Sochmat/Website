...
/**
 * Blank upload template. Lists the live raw-material, production-item and
 * item-recipe names on an Instructions sheet, split by the Type value each one
 * needs.
 */
export async function GET() {
  try {
    const [materials, items, recipes] = await Promise.all([
      listRawMaterials(),
      listProductionItems(),
      listItemRecipes(),
    ]);
