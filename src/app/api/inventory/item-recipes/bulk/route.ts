...
/**
 * Commit an item-recipe import plan.
 *
 ...
 * let a crafted request write an arbitrary cost or an unknown component. The
...
export async function POST(request: NextRequest) {
  try {
    ...
    const build = (raw: unknown, id?: string) => {
      ...
      // Upsert on nameKey: if the recipe was created by someone else between
    ...
    console.error("Error committing item recipe import:", error);
    return NextResponse.json(
      { success: false, message: "Failed to import item recipes" },
      { status: 500 },
    );
  }
}
