...
export default function AdminMealCardsPage() {
  ...
  const fetchCards = async () => {
    try {
      const res = await fetch("/api/admin/meal-cards");
      const data = await res.json();
      if (data.success) setCards(data.cards);
    } catch (err) {
      console.error("Failed to fetch meal cards:", err);
    }
  };
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    if (!form.title.trim() || validImages.length === 0) return;
    setLoading(true);
    try {
      const payload = {
        title: form.title,
      ...
      if (editingId) {
        await fetch("/api/admin/meal-cards", {
          method: "PUT",
  ...
  const handleEdit = (card: MealCard) => {
  ...
  const handleDelete = async (id: string) => {
    if (!confirm("Delete this meal card?")) return;
    try {
      await fetch(`/api/admin/meal-cards?id=${id}`, { method: "DELETE" });
      fetchCards();
    } catch (err) {
  ...
  const field = (
    label: string,
    key: "title" | "subtitle" | "startingPrice" | "link" | "order",
    type = "text",
    placeholder = ""
  ) => (
    <div>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
        ...
        <div className="bg-white rounded-xl shadow-lg p-6">
          <h2 className="text-lg font-bold text-gray-800 mb-4">
            {editingId ? "Edit Meal Card" : "Add Meal Card"}
          </h2>
                ...
                {form.images.map((url, idx) => (
                  <div key={idx} className="flex gap-2">
                    ...
                    <button
                      ...
                      className="px-3 py-2 text-red-600 hover:bg-red-50 rounded-lg transition-colors border border-gray-300"
                    >
                      <svg
                        className="w-4 h-4"
                        fill="none"
            ...
            {form.images.filter((u) => u.trim()).length > 0 && (
              <div className="rounded-lg overflow-hidden h-40 bg-gray-100 relative">
                ...
                <div className="absolute bottom-2 right-2 text-right bg-white/80 rounded px-2 py-0.5">
                  <p className="text-[#444] text-[9px] uppercase tracking-wide">
                    Starting at
                  </p>
        ...
        <div className="bg-white rounded-xl shadow-lg p-6">
          <h2 className="text-lg font-bold text-gray-800 mb-4">
            Meal Cards ({cards.length})
          </h2>
          <div className="space-y-3 max-h-[600px] overflow-y-auto">
            {cards.length === 0 ? (
              <p className="text-gray-500 text-center py-8">
                No meal cards yet. Add your first card!
              </p>
            ) : (
              cards.map((card) => (
                <div
                ...
                >
                  ...
                  <div className="flex-1 min-w-0">
                    ...
                    <div className="flex items-center gap-2 mt-0.5">
                      <span className="text-xs text-[#f28a1d] font-semibold">
                        ₹{card.startingPrice}
                      </span>
                      <span className="text-xs text-gray-400">
                        · {card.images.length} image
